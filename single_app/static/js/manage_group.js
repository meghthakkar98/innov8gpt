...
$(document).ready(function () {
  ...
  $("#addMemberBtn").on("click", function () {
    ...
    $("#newUserEmail").val("");
    $("#customInviteMessage").val(""); // Clear custom message field
    $("#searchStatus").text("");
  ...
  // Add custom message field to the Add Member modal
  // This should be done after the modal is in the DOM
  const modalBody = document.querySelector('#addMemberModal .modal-body');
  if (modalBody) {
    // Find the email field container
    const emailField = modalBody.querySelector('#newUserEmail');
    if (emailField && emailField.parentNode) {
      // Check if custom message field already exists to avoid duplicates
      if (!document.getElementById('customInviteMessage')) {
        const customMessageDiv = document.createElement('div');
        customMessageDiv.className = 'mb-2';
        customMessageDiv.innerHTML = `
          <label for="customInviteMessage" class="form-label">Invitation Message (Optional)</label>
          <textarea class="form-control" id="customInviteMessage" rows="3" 
                    placeholder="Add a personal message to the invitation email..."></textarea>
        `;
        
        // Insert after the email field's parent div
        emailField.parentNode.insertAdjacentElement('afterend', customMessageDiv);
      }
    }
  }
});
...
function loadPendingRequests() {
  $.get(`/api/groups/${groupId}/requests`, function (pending) {
    ...
    pending.forEach((u) => {
      rows += `
        <tr>
          ...
          <td>${u.email}</td>
          <td>
            <button class="btn btn-sm btn-success approve-request-btn" data-user-id="${u.userId}">Approve</button>
    ...
    $('.approve-request-btn').on('click', function() {
      ...
      openApproveRequestModal(userId);
    });
...
// New function to open approve request modal
function openApproveRequestModal(userId) {
  // Create the modal if it doesn't exist
  const modalId = 'approveRequestModal';
  ...
  if (!modal) {
        ...
        <form id="approveRequestForm">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="${modalId}Label">Approve Join Request</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <input type="hidden" id="approveUserId" />
              <div class="mb-3">
                <label for="approveMessage" class="form-label">Welcome Message (Optional)</label>
                <textarea class="form-control" id="approveMessage" rows="3" 
                          placeholder="Add a welcome message to the approval notification..."></textarea>
              </div>
            </div>
    ...
    document.getElementById('approveRequestForm').addEventListener('submit', function(e) {
      ...
      const customMessage = document.getElementById('approveMessage').value;
      
      approveRequest(approveUserId, customMessage);
...
function approveRequest(requestId, customMessage = '') {
  $.ajax({
    ...
    success: function () {
      loadMembers();
      loadPendingRequests();
      showToast("Request approved and notification sent!", "success");
    },
    error: function (err) {
      console.error(err);
...
function addMemberDirectly() {
  ...
  const customMessage = $("#customInviteMessage").val().trim(); // Get custom invitation message
  ...
  $.ajax({
    ...
    data: JSON.stringify({ 
      ...
      displayName, 
      email,
      customMessage // Include the custom message
    }),
    success: function () {
