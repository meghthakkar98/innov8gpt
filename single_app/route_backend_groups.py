...
from functions_email import send_group_invitation_email
...
def register_route_backend_groups(app):
    ...
    def api_create_group():
        ...
        try:
            ...
            if 'roles' not in user or 'Admin' not in user['roles']:
                return jsonify({"error": "Only administrators can create new channels"}), 403
    ...
    # Update the approve_reject_request function to send email on approval
    ...
    def approve_reject_request(group_id, request_id):
        ...
        if action == "approve":
            ...
            if member_to_add.get("email"):
                try:
                    Thread(
                        target=send_group_invitation_email,
                        args=(
                            member_to_add["email"],
                            member_to_add["displayName"],
                            group_doc["name"],
                            user_info["displayName"],
                            role,
                            group_id,
                            custom_message or "Your request to join has been approved."
                        )
                    ).start()
    ...
    def add_member_directly(group_id):
        ...
        if new_member_doc["email"]:
            custom_message = data.get("customMessage")
            inviter_name = user_info["displayName"]
            inviter_role = role
            ...
            try:
                Thread(
                    target=send_group_invitation_email,
                    args=(
                        new_member_doc["email"],
                        new_member_doc["displayName"],
                        group_name,
                        inviter_name,
                        inviter_role,
                        group_id,
        ...
        return jsonify({"message": "Member added and invitation sent"}), 200
