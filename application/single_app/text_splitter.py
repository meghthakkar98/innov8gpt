...
# Define the Page and SplitPage classes
class Page:
    """A simple class to represent a page of text."""
    def __init__(self, text: str, page_num: int, offset: int = 0):
        self.text = text
        self.page_num = page_num
        self.offset = offset

class SplitPage:
    """A class to represent a split portion of a page."""
...
class TextSplitter:
    ...
    def split_pages(self, pages: List[Page]) -> Generator[SplitPage, None, None]:
        if False:
            yield  # pragma: no cover - necessary for mypy to type check


class SentenceTextSplitter(TextSplitter):
    """
    Class that splits pages into smaller chunks based on sentence boundaries.
